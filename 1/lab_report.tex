...
When, we increase the sampling frequency to 39.8 kHz, the samples are periodic with period that is 10x that of the sampled sinusoid. The period of the sampled signal is greater than the original sinusoid because the sampling frequency is not evenly divisible by the sinusoid's carrier frequency. The constellation map with a sampling rate of 39.8 kHz is shown in Figure \ref{fig::iq_imbalance_0_mag_0_phase_39p8k_samp_rate_const}.
