%% Detect start of frame with xcorr
...
%% Detect start of frame with filter
filt = filter(flip(conj(seq)),1,y);
[v,i] = max(abs(filt));
...
subplot(2,1,2);title('Matched Filter Output');xlabel('Samples');
hold on;
h3 = plot(filt);grid on;
h6 = stem(i,v);
hold off;

% Estimation of peak position
peakPosEst = gapLen + (sequenceLength - 1) + 1;
hold on;
h4 = stem(peakPosEst,-v);
legend([h6,h4],'True Peak Position','Estimated Peak Position','Location','Best');
hold off;
...
% Estimate 'gapLen' with known BarkerLength and Number of starting zeros
[~,p] = max(filt);
estGapLen = p - (sequenceLength - 1) - 1;
subplot(2,1,1);
hold on;
h5 = stem(estGapLen,-2);
