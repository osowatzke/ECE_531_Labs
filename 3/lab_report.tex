...
Next, we looked at the LTE spectrum in the 700 MHz A spectrum. To do this, we set our center frequency to 731.5 MHz and our bandwidth to 8 MHz. The resulting spectrum is shown in Figure \ref{fig::lte_spectrum}.
...
For the final modulation scheme, we examined the private carrier paging (PCP) networks, which lie between 929 and 930 MHz. 
...
Examining the spectrum, we see that it alternates between 2 discrete frequencies and maintains a constant power when it is on. As such, we conclude that the PCP network uses FSK modulation.
