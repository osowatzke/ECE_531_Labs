...
In GNU Radio, we also used the provided flowchart to measure the receiver gain that resulted in saturation. After replacing the "QT Time" and "QT Frequency" sink with the "QT GUI" sink, we were given an additional averaging option. This option averaged the FFT outputs in power resulting in a reduced noise variance and a better estimate of the signal's frequency response. We also collected the frequency response with a rectangular window instead of a Blackmann-Harris window. This resulted in a reduced mainlobe width at the cost of increased sidelobe levels.

Then, we replaced the PlutoSDR Sink and Source block with equivalent generic IIO blocks. This gave us an opportunity to learn more about the IIO control of the \textit{ad9361-phy}. To validate our work, we repeated the GNU Radio loopback experiment used the updated flowchart. We also updated the LO frequency and sampling rate in the flowchart and validated that these parameters were correctly applied with the \texttt{iio\_attr} command.

Finally, we used MATLAB loopback collects to measure the SNR of the signal. With \textit{Manual} AGC, our SNR increased linearly as we increased the transmitted signal power. We also observed an SNR gain for fixed transmit powers when increasing the receiver gain. This occurred due to due Friss' formula, which says that the noise figure decreases when we apply our gain early in a cascade of amplifiers. We also tried measuring the SNR with \textit{AGC Slow Attack} and \textit{AGC Fast Attack} enabled. These AGC algorithms increased the power of the noise outside of the transmit pulse reducing our measured SNR.

%IIO is a standard interface for interacting with the PlutoSDR. The MATLAB and GNU Radio APIs both leverage it to interface with the device. After learning about IIO, we performed loopback collects in MATLAB and GNU radio. In MATLAB, we measured the levels of receiver gain, which led to saturation. We then experimented with the \textit{AGC Fast Attack} and \texttt{AGC Slow Attack} Algorithm. We saw that the \textit{AGC Fast Attack} algorithm responded faster to rapidly changing inputs. However, we also observed distortion due to both AGC algorithms (transients and clipping).  both led to distortion when the gain rapidly changed. However  First, in MATLAB, we experiment with the AGC settings and determine the gain needed to saturate the receiver. Next, in GNU radio, we evaluate the parameters of the built-in PlutoSDR blocks and once again determine the gain needed to saturate the receiver. We also observe the frequency response and learn how to map the input frequency to the RF frequency. Next, we create our own PlutoSDR blocks from IIO primitives and vaidate their behavior. Finally, we use MATLAB to measure the SNR of the received signal for different gain levels and AGC settings. 
